...
#subsetting and cutoff minimum nights over 14 days
...
# dataset_1 = pd.get_dummies(dataset, columns=["neighbourhood_group","room_type"], prefix = ["nbhdg","rt"])
# #remove drop_first = TRUE argument so that we have N number of dummies for N level
...
dataset_1 = pd.get_dummies(dataset, columns=["room_type", "neighbourhood"], prefix = ["rt","nbhd"])
...
print(machine.coef_)
print(1-(1-machine.score(x_train1,y_train1))*(len(y_train1)-1)/(len(y_train1) - x_train1.shape[1]-1))
predictions = machine.predict(x_test1)
print(predictions)
